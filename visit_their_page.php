<?php
/**
* Visited their page users listing
*
* @package DatingPro
...
switch ($sel) {
	case 'addhotlist':
		...
		if (REDIRECT_AFTER_ACTION) {
			...
			header('Location: visit_their_page.php?par=send');
			exit;
		} else {
			$_GET['par'] = 'send';
			SearchTable($res['err']);
...
function SearchTable($err = '')
{
	...
	$file_name = 'visit_their_page.php';
		...
		$strSQL =
		 ...
		 INNER JOIN '.PROFILE_VISIT_TABLE.' l ON l.id_user = u.id
		  ...
		  LEFT JOIN '.USER_PRIVACY_SETTINGS.' up ON up.id_user = u.id
		   ...
		   GROUP BY u.id
		   ORDER BY MAX(l.visit_date) DESC';
	...
	if ($num_records > 0)
	{
		...
		$strSQL =
			'SELECT u.id, u.fname, u.phone, SUBSTRING(u.comment, 1, 165) AS comment, u.gender,
			   ...
			   FROM '.USERS_TABLE.' u
		 INNER JOIN '.PROFILE_VISIT_TABLE.' l ON l.id_user = u.id AND l.id_visiter = ?
...
#			$search[$i]['completion']	= $percent->GetAllPercentForUser($row['id']);
	...
	$smarty->display(TrimSlash($config['index_theme_path']).'/visit_their_page_table.tpl');
	exit;
}
?>