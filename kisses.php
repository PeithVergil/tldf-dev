<?php
/**
* User Send-Kiss file (sended kisses listing, add to hot and black list)
*
* @package DatingPro
...
switch ($sel) {
	case 'addhotlist':
		...
		if (REDIRECT_AFTER_ACTION) {
			...
			header('Location: kisses.php?sel='.$sel.'&par=send');
			exit;
		} else {
			$_GET['par'] = 'send';
...
#	$percent = new Percent($config, $dbconn, $id_user);
	...
	*/
		...
		if ($sel == 'me') {
			...
			$strSQL =
			 ...
			 INNER JOIN '.KISSLIST_TABLE.' l ON l.id_from = u.id
			  ...
			  LEFT JOIN '.USER_PRIVACY_SETTINGS.' up ON up.id_user = l.id_from
				  WHERE l.id_to = ? AND l.id_from <> ?
					AND u.status = "1" '.$visible_where.' '.$privacy_where.'
			   GROUP BY u.id
			   ORDER BY MAX(l.kiss_date) DESC';
			...
			$strSQL =
			 ...
			 INNER JOIN '.KISSLIST_TABLE.' l ON l.id_to = u.id
			  ...
			  LEFT JOIN '.USER_PRIVACY_SETTINGS.' up ON up.id_user = l.id_to
				  WHERE l.id_from = ? AND l.id_to <> ?
					AND u.status = "1" '.$visible_where.' '.$privacy_where.'
			   GROUP BY u.id
			   ORDER BY MAX(l.kiss_date) DESC';
			$paramsSQL = array($id_user, $id_user);
	...
	if ($num_records > 0)
	{
		...
		$join_field = ($sel == 'me') ? 'id_from' : 'id_to';
		$where_field = ($sel == 'me') ? 'id_to' : 'id_from';
		...
		$strSQL =
			'SELECT u.id, u.fname, u.phone, SUBSTRING(u.comment, 1, 165) AS comment, u.gender,
			   ...
			   FROM '.USERS_TABLE.' u
		 INNER JOIN '.KISSLIST_TABLE.' l ON l.'.$join_field.' = u.id AND '.$where_field.' = ?
...
#			$search[$i]['completion']	= $percent->GetAllPercentForUser($row['id']);
			...
			// list of kisses
			if ($sel == 'me') {
				$kisslist_where = 'a.id_from = '.$row['id'].' AND a.id_to = '.$id_user;
			} else {
				$kisslist_where = 'a.id_from = '.$id_user.' AND a.id_to = '.$row['id'];
			}
			
			$ml = new MultiLang();
			
			$sub_strSQL =
				'SELECT DISTINCT c.'.$ml->DefaultFieldName().' AS name, b.image_path,
						DATE_FORMAT(a.kiss_date, "'.$config['date_format'].'") AS date
				   FROM '.KISSLIST_TABLE.' a
			  LEFT JOIN '.KISSLIST_SPR_TABLE.' b ON a.kiss_type = b.id
			  LEFT JOIN '.REFERENCE_LANG_TABLE.' c ON c.table_key = "'.$ml->TableKey(KISSLIST_SPR_TABLE).'" AND c.id_reference = b.id
				  WHERE '.$kisslist_where.'
			   ORDER BY a.id DESC
			      LIMIT 0, 10';
			...
			if ($sub_rs->RowCount() > 0) {
				while (!$sub_rs->EOF) {
					$sub_row = $sub_rs->GetRowAssoc(false);
					$kisses[$k]['name'] = stripslashes($sub_row['name']);
					if ($sub_row['image_path'] && file_exists($config['site_path'].$settings['kiss_folder'].'/'.$sub_row['image_path'])){
						$kisses[$k]['image_path'] = $config['site_root'].$settings['kiss_folder'].'/'.$sub_row['image_path'];
					}
					$kisses[$k]['date'] = $sub_row['date'];
					$sub_rs->MoveNext();
					$k++;
				}
				$search[$i]['kisses'] = $kisses;
			}
		...
		// paging links
		$param = $file_name.'?sel='.$sel.'&amp;filter='.$filter.'&amp;view='.$view.'&amp;';
	...
	$smarty->display(TrimSlash($config['index_theme_path']).'/kisses_table.tpl');
	exit;
}
?>