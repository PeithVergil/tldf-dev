...
switch ($sel) {
	case 'addhotlist':
		...
		if (REDIRECT_AFTER_ACTION) {
			...
			header('Location: emailed_me.php?par=send');
			exit;
		} else {
			$_GET['par'] = 'send';
			SearchTable($res['err']);
	...
	if ($num_records > 0)
	{
		...
		$strSQL =
			'SELECT u.id, u.fname, u.phone, SUBSTRING(u.comment, 1, 165) AS comment, u.gender,
			   ...
			   FROM '.USERS_TABLE.' u
		 INNER JOIN '.MAILBOX_TABLE.' l ON l.id_from = u.id AND l.id_to = ? AND l.deleted_to = "0"
...
#			$search[$i]['completion']	= $percent->GetAllPercentForUser($row['id']);
	...
	$smarty->display(TrimSlash($config['index_theme_path']).'/emailed_me.tpl');
	exit;
}
?>