// RS: used in admin_answers_table.tpl, admin_events_users_form.tpl, admin_groups_users_form
...
function _a_include(src, path, ver){
	...
	var strJS = "<SCRIPT LANGUAGE=\"JavaScript";
	var strEJS = "\"></SCRIPT>";
...
function _q_removeField(field){
	...
	if( _jsver == 12 ){
		delete this[field];
		delete this._fields[this._fields.length];
		delete this._pointers[field.toLowerCase()];
	} else {
		this[field] = void(0);
		this._fields[this._fields.length] = void(0);
		this._pointers[field.toLowerCase()] = void(0);
	}
	return true;
}
		...
		if( this.type == "select-one" && !this.container ){
			...
			for( var i=0; i < this.obj.length; i++ ){
				...
				if( (this.obj[i].selected || this.container) && (!this.dummyContainer) ){
					...
					value[value.length] = (!!this.obj[i].value) ? this.obj[i].value : this.obj[i].text;
				}
			}
		}
...
function _f_setValue(value, bReset, doBlur){
	...
	var triggerEvent = _param(arguments[3], null, "string");
	var type = (this.type.substring(0,6) == "select") ? "select" : this.type;
	...
	if( type == "select" ){
		...
		if( !this.container ){
			// loop through all element in the array for this field
			for( var i=0; i < this.obj.length; i++ ){
				var v = (!!this.obj[i].value) ? this.obj[i].value : this.obj[i].text;
	...
	if( !!doBlur ) this.triggerEvent("onblur");
...
function _addValidator(name, fn, alwaysRun){
	...
	strRule += "this.addEvent('onblur', this.pointer + '.checkForErrors()');";
	strRule += "return true;\n";
	...
	for( var i=0; i < obj._fields.length; i++ ){
		if( obj[obj._fields[i]].container && obj[obj._fields[i]].type.substring(0,6) == "select" ){
			for( var x=0; x < obj[obj._fields[i]].obj.length; x++ ){
				if(!obj[obj._fields[i]].dummyContainer){
					str += obj[obj._fields[i]].obj[x].value + ",";
					obj[obj._fields[i]].obj[x].value = str;
				}
...
function _Field_ltrim(){
	this.setValue(_ltrim(this.getValue()));
	return true;
}
...
function _Field_rtrim(){
	this.setValue(_rtrim(this.getValue()));
	return true;
}
...
function _Field_trim(){
	this.setValue(_trim(this.getValue()));
	return true;
}
...
function _Field_mirrorTo(objName){
	...
	if( isQForm ){
		var strCommand = objName  + ".setValue(" + this.pointer + ".getValue()" + ");";
...
function _Field_createDependencyTo(field, condition){
	...
	// add an onblur event so that when the field is updated, the requested field
	// is updated with the value
	if( this.qForm[field]._queue.dependencies.length == 0 ) this.qForm[field].addEvent(_getEventType(this.name), otherField + ".enforceDependency();", false);
...
function _Field_isDependent(field, condition){
	...
	if( condition == null ){
		...
		// otherwise, you're doing a simple value compare
			...
			var result = (this.getValue() == condition);
		}
	}
...
function _Field_location(target, key){
	...
	if( this.isLocked() || this.isDisabled() ) return this.setValue(key); 

	var value = this.getValue();
	this.setValue(key);
	// if the value isn't equal to the key, then don't relocate the user
	if( value != key ){
		eval(target + ".location = value");
	}
	return true;
}
...
/////////////////////////////////////////////////////////////////////////////
function ChangePAR(form_name, conf_str){
//	if(confirm(conf_str)){
		document.forms[form_name].submit();	//////  отправляем форму
//	}
}
/////////////////////////////////////////////////////////////////////////////
