...
YAHOO.widget.Node.prototype = {
	...
	document.cookie = 'show' + this.cookieid + "=N; path=/; expires=Thu, 31 Dec 2020 23:59:59 GMT;";
	...
	if (this.hasChildren()) {
		document.cookie = 'show' + this.cookieid + "=Y; path=/; expires=Thu, 31 Dec 2020 23:59:59 GMT;";
	}
...
/**
 * The CSS class for the depth label href.  Defaults to ygtvdepth, but can be
 * overridden to provide a custom presentation for a specific node.
 *
 * @type string
 */
YAHOO.widget.TextNode.prototype.depthStyle = "ygtvdepth";
 ...
 * @type string
 */
YAHOO.widget.TextNode.prototype.icon = "";

/**
 *
 * @type string
 */
YAHOO.widget.TextNode.prototype.cookieid = "";

/**
 *
 * @type string
 */
YAHOO.widget.TextNode.prototype.onclick = "";
...
YAHOO.widget.TextNode.prototype.setUpLabel = function(oData) {
	...
	// update the cookieID
	if (oData.cookieid) {
		this.cookieid = oData.cookieid;
	}

	if (oData.onclick) {
		this.onclick = oData.onclick;
	}
...
YAHOO.widget.TextNode.prototype.getNodeHtml = function() {
	...
	sb[sb.length] = ' onclick="javascript: ' + this.getToggleLink() + '" valign="middle">';
//	sb[sb.length] = '</td>';
//	sb[sb.length] = '<td>';
	/*sb[sb.length] = '<table cellspacing="0" cellpadding="0"><tr><td valign="middle" style="padding-left: 6px; width: 22px; background: url(' + this.icon + ') 6px 50% no-repeat;">&nbsp;</td><td valign="middle"><a';
	sb[sb.length] = ' id="' + this.labelElId + '"';
	if (this.depth)
		sb[sb.length] = ' class="' + this.depthStyle + '"';
	else
		sb[sb.length] = ' class="' + this.labelStyle + '"';
	if (this.onclick) {
		sb[sb.length] = ' href="#" onClick="' + this.onclick + '"';
	} else {
	...
	*/
		...
		sb[sb.length] = '<table cellspacing="0" cellpadding="0"><tr><td valign="middle" style="padding-left: 6px; width: 22px; background: url(' + this.icon + ') 6px 50% no-repeat;">&nbsp;</td><td valign="middle"><span';
		sb[sb.length] = ' id="' + this.labelElId + '"';
		if (this.depth)
			sb[sb.length] = ' class="' + this.depthStyle + '"';
		else
