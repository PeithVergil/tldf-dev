...
function TCRisPoint(tmp,n) {
	var flag = 0;
	if (tmp == '.') {
		var tmp2 = document.forms[0].elements[0].value;
...
function TCRmntr(num) {
	var flag = 0;
	if (this.t_load) tmp = document.forms[0].elements[0].value;
	if (num == 'C') {
		this.out_val = '0';
	...
	if (this.t_load) document.forms[0].elements[0].value = this.out_val;
}

function TCRNew(obj_control) {
	this.control_obj = obj_control;
	return;
}