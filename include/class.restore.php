...
class BaseRestore {
	...
	function restore($db_host, $db_uname, $db_passw, $db_name, $file){
		...
		foreach($file_content_str as $str){
			if(!mysql_query($str)){
				print tpl_l(htmlspecialchars("Query is not valid."));
		...
		print tpl_l("Total db queries: {$q}", C_RESULT);
		insertLogData("Total db queries: {$q}.");
		print tpl_l("Tables created: {$tabs}", C_RESULT);
		insertLogData("Tables created: {$tabs}.");
		print tpl_l("Lines added : {$aff_rows}", C_RESULT);
		insertLogData("Lines added : {$aff_rows}.");
