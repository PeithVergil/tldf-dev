...
**/
  ...
  /* ================================================== \\
	...
	:...> Modified: tselischev@pilotgroup.net
  \\ ================================================== */
  ...
  class rssGenesis {
    ...
    var $rss_channel = "\t<channel>\r\n\t\t<title><![CDATA[{channel_Title}]]></title>\r\n\t\t<link><![CDATA[{channel_Link}]]></link>\r\n\t\t<description><![CDATA[{channel_Description}]]></description>\r\n\t\t<language><![CDATA[{channel_Language}]]></language>\r\n\t\t<copyright><![CDATA[{channel_Copyright}]]></copyright>\r\n\t\t<managingEditor><![CDATA[{channel_ManagingEditor}]]></managingEditor>\r\n\t\t<webMaster><![CDATA[{channel_WebMaster}]]></webMaster>\r\n\t\t<rating><![CDATA[{channel_Rating}]]></rating>\r\n\t\t<pubDate><![CDATA[{channel_PubDate}]]></pubDate>\r\n\t\t<lastBuildDate><![CDATA[{channel_LastBuildDate}]]></lastBuildDate>\r\n\t\t<category><![CDATA[{channel_Category}]]></category>\r\n\t\t<generator><![CDATA[RSS GENERATOR]]></generator>\r\n\t\t<docs><![CDATA[{channel_Docs}]]></docs>\r\n\t\t<skipDays>{channel_SkipDays}</skipDays>\r\n\t\t<skipHours>{channel_SkipHours}</skipHours>\r\n\r\n";
    ...
    var $rss_input = "\t\t<textinput>\r\n\t\t\t<title><![CDATA[{input_Title}]]></title>\r\n\t\t\t<description><![CDATA[{input_Description}]]></description>\r\n\t\t\t<name><![CDATA[{input_Name}]]></name>\r\n\t\t\t<link>{input_Link}</link>\r\n\t\t\t</textinput>\r\n\r\n";
	...
	function rssGenesis ($config, $rssversion = "RSS20" ) {
		$this->rssformat = $rssversion;
		$this->config = $config;
		if ($rssversion == "RSS091")
		{
			$this->rss_header = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n\r\n<!DOCTYPE rss PUBLIC \"-//Netscape Communications//DTD RSS 0.91//EN\" \"http://my.netscape.com/publish/formats/rss-0.91.dtd\">\r\n\r\n<rss version=\"0.91\">\r\n";
    ...
    function setChannel ($config, $title = "", $link = "", $description = "", $language = "", $copyright = "", $managingEditor = "", $webMaster = "", $rating = "", $pubDate = "", $lastBuildDate = "", $category = "", $docs = "", $timetolive = "", $skipDays = "", $skipHours = "") {
      ...
      $title = (empty ($title)) ? "RENTMATE.RU News" : $title;

      // Link parser // Checks absolutes URIs
      if (!preg_match ("(^(ht|f)tp://)", $link)) :

        $link = $config["site_root"];
      ...
      if (!preg_match ("(^(ht|f)tp://)", $docs)) :
        ...
        $docs = $config["server"].$config["site_root"];
    ...
    function setImage ($config, $title = "", $src = "", $link = "", $width = "", $height = "", $description = "") {
    ...
    $this->config = $config;
      ...
      // Description parser // Empty descriptions not allowed
      $description = (empty ($description)) ? "Powered by: RSS RENTMATE.RU!" : $description;
    ...
    function addItem ($title, $link, $description, $pubdate = "", $category = "", $guid = "") {
	  ...
	  //Formats pubdate
	  if ($pubdate=='auto') {
	  	$pubdate = gmdate('D, d M Y H:i:s \G\M\T', time());
	  } else {
      ...
      if (empty ($title)) :
        ...
        die ("<font face=\"verdana\" size=\"2\">Error code: <strong>001</strong> - Item's element title is mandatory! - <a href=\"http://boss/yurasites/rentmate/index.php\" target=\"_blank\">Documentation</a></font>");
      ...
      if (!preg_match ("(^(ht|f)tp://)", $link)) :
        ...
        die ("<font face=\"verdana\" size=\"2\">Error code: <strong>002</strong> - Item's element link is mandatory! - <a href=\"http://boss/yurasites/rentmate/index.php\" target=\"_blank\">Documentation</a></font>");
	  ...
	  if ($this->rssformat == "RSS20") {
			...
			$temp = ereg_replace ("{item_Guid}", $guid, $temp);
	  }
    ...
    function setInput ($config, $title = "", $description = "", $name = "", $link = "") {
    ...
    function generateFeed ($config) {

      // Calls the function to organize data //
      $this->organizeData();
