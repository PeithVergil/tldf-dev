...
/**
* Ogone payment system functions
*
* @package DatingPro
...
//			'accept_url'    => $config["server"].$config["site_root"]."/include/payment_request.php?sel=ogone",
...
function RequestPayment($billing_system, $__REQUEST) {
	...
	$PaySystem = &$payGear->factory($billing_system);
	$pay_data = array(
		'count'		=> $__REQUEST[$PaySystem->_arrayMap['amount']]/100,
...
function setBillingData($billing_system, $__POST) {
	global $dbconn, $lang;
		...
		if($use!=0 && !$value){
			...
			$err .= "<br>".$lang["pays"]["ogone_seller_id"];
		} else {
