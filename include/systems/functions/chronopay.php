...
/**
* ChronoPay payment system functions
*
* @package DatingPro
...
function MakePayment($billing_system, $amount, $currency, $id_trunzaction, $product_name, $adv_arr='') {
	global $dbconn, $config;
		...
		$pay_data = array(
			'product_id'     => $rs_sys->fields[0],
			'amount'        => $amount,
			'order_id'		=> $id_trunzaction,
			'notify_url'    => $config["server"].$config["site_root"]."/include/payment_request.php?sel=chronopay",
			'decline_url' 	=> $config["server"].$config["site_root"]."/account.php",
...
function RequestPayment($billing_system, $__REQUEST) {
	...
	$PaySystem = &$payGear->factory($billing_system);
	$pay_data = array(
		'count'		=> $__REQUEST[$PaySystem->_arrayMap['total']],
		'currency'	=> $__REQUEST[$PaySystem->_arrayMap['currency']],
		'date'		=> date("Y-m-d H:i:s", strtotime($__REQUEST[$PaySystem->_arrayMap['payment_date']])),
...
function setBillingData($billing_system, $__POST) {
	global $dbconn, $lang;
		...
		if($use != 0 && !$value){
			...
			$err .= "<br>".$lang["pays"]["chronopay_product_id"];
		} else {
