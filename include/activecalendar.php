...
var $cssMonthLink="monthlink"; // a tag: day
...
var $cssNotEmptyDay="notemptyday"; // td tag: the selected day
...
var $cssDayLink="daylink"; // a tag: day
var $cssTodayLink="todaylink"; // a tag: today
var $cssSelectDayLink="selectdaylink"; // a tag: today
...
********************************************************************************
*/
function activeCalendar($year=false,$month=false,$day=false,$GMTDiff="none", $lang=false){
	$this->timetoday=time();
	$this->selectedday=-2;
	$this->notemptyday=-2;
	...
	if(is_array($lang)){
		$this->jan=$lang["blog"]["month"][1];
		$this->feb=$lang["blog"]["month"][2];
		$this->mar=$lang["blog"]["month"][3];
		$this->apr=$lang["blog"]["month"][4];
		$this->may=$lang["blog"]["month"][5];
		$this->jun=$lang["blog"]["month"][6];
		$this->jul=$lang["blog"]["month"][7];
		$this->aug=$lang["blog"]["month"][8];
		$this->sep=$lang["blog"]["month"][9];
		$this->oct=$lang["blog"]["month"][10];
		$this->nov=$lang["blog"]["month"][11];
		$this->dec=$lang["blog"]["month"][12];
		$this->sun=$lang["blog"]["sun"];
		$this->mon=$lang["blog"]["mon"];
		$this->tue=$lang["blog"]["tue"];
		$this->wed=$lang["blog"]["wed"];
		$this->thu=$lang["blog"]["thu"];
		$this->fri=$lang["blog"]["fri"];
		$this->sat=$lang["blog"]["sat"];
	}
}
/*
********************************************************************************
...
********************************************************************************
*/
function showMonth($showNoMonthDays=false, $notempty_array=false){
...
$out.=$this->mkWeekDays(); // tr tag: the weekday names
	if ($this->showNoMonthDays==false) $out.=$this->mkMonthBody(0, $notempty_array); // tr tags: the days of the month
	else $out.=$this->mkMonthBody(1, $notempty_array); // tr tags: the days of the month
...
function mkMonthTitle(){
	if (!$this->monthNav){
		$out="<tr>\n\t";
		$out.="<td class=\"".$this->cssMonthTitle."\" colspan=\"7\">";
		...
		$out.="<td class=\"".$this->cssMonthTitle."\" colspan=\"5\">";
...
function mkWeekDays(){
	if ($this->startOnSun){
		...
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(1)."</td>\n\t";
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(2)."</td>\n\t";
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(3)."</td>\n\t";
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(4)."</td>\n\t";
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(5)."</td>\n\t";
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(6)."</td>\n</tr>\n";
	}
	else{
		$out="<tr>\n\t";
		$out.="<td class=\"".$this->cssWeekDay."\">".$this->getDayName(1)."</td>\n\t";
...
********************************************************************************
*/
function mkMonthBody($showNoMonthDays=0, $notempty_array=false){
	if ($this->actmonth==1){
		$pMonth=12;
	...
	for ($x=0; $x<=6; $x++){
		if ($x>=$this->firstday){
			$monthday++;
			if(is_array($notempty_array) && in_array($monthday, $notempty_array) && count($notempty_array) >0) $this->notemptyday = $monthday;
	...
	for ($x=0; $x<=6; $x++){
		...
		$out.="<tr>";
			...
			if(is_array($notempty_array) && in_array($i, $notempty_array) && count($notempty_array) >0) $this->notemptyday = $i;
					$out.=$this->mkDay($i);
				}
			}
		$goon=$goon+7;
...
$linkstr=$this->mkUrl($this->actyear,$this->actmonth,$var);
	...
	elseif ($var==$this->notemptyday && $this->actmonth==$this->selectedmonth && $this->actyear==$this->selectedyear){
		if (!$this->dayLinks) $out="<td class=\"".$this->cssNotEmptyDay."\">".$var.$eventContent."</td>\n\t";
		else $out="<td class=\"".$this->cssNotEmptyDay."\">".$linkstr.$eventContent."</td>\n\t";
	}
...
$monthNavLink="<a href=\"".$this->urlNav.$glueNav.$this->yearID."=".$year."&amp;".$this->monthID."=".$month."\" class=\"".$this->cssMonthLink."\">";
$dayLink="<a href=\"".$this->url.$glue.$this->yearID."=".$year."&amp;".$this->monthID."=".$month."&amp;".$this->dayID."=".$day."\" class=".$this->cssDayLink.">".$day."</a>";
