...
class Browser_Detection
{
	 ...
	 * @access public
	 */
	static function get_browser($useragent)
	{
		...
		elseif (strpos($useragent, "Gecko"))
		{
			...
			//if firefox
			$found = preg_match("/Firefox\/([0-9]{1,2}\.[0-9]{1}(\.[0-9])?)/", $useragent, $matches);
			if ($found)
			{
			...
			$found = preg_match("/Chrome\/([^\s]+)/", $useragent, $matches);
			if ($found)
			{
		...
		{
			//deal with Opera
			$found = preg_match("/Opera[\/ ]([0-9]{1,2}\.[0-9]{1,2})/", $useragent, $matches); // ([0-9])?
			if ($found)
			{
	 ...
	 * @access public
	 */
	static function get_os($useragent)
	{		
		...
		elseif (strpos($useragent, "win 9x 4.90") !== false && strpos($useragent, "win me") !== false)
		{
			return "Windows ME";
		}
