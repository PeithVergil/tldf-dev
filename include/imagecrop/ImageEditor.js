	...
	request.onreadystatechange = function(){
		if (request.readyState == 4){
			...
			if (json.imageFound) {
				...
				ImageEditor.max_w = json.max_w;
				ImageEditor.max_h = json.max_h;
...
ImageEditor.save = function(){
	if (ImageEditor.w > ImageEditor.max_w || ImageEditor.h > ImageEditor.max_h){
		alert("Width cannot exceed " + ImageEditor.max_w + " pixels and/or height cannot exceed " + ImageEditor.max_h + " pixels.");
		return;
	}
	ImageEditor.processImage("action=save");
	opener.window.location.href = "../../myprofile.php?sel=4";
};
...
ImageEditor.init = function(imageName){
	...
	ImageEditor.loaderImage.onload = function(){ 
		ImageEditor.displayImage();
		document.getElementById("ImageEditorNotice").innerHTML = "Notice: Thumb's width cannot exceed " + ImageEditor.max_w + " pixels and/or height cannot exceed " + ImageEditor.max_h + " pixels.";
	};
