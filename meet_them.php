<?php
/**
* User want-to-meet-them management, add to hot and black list
*
* @package DatingPro
...
switch ($sel) {
	case 'kiss':
		...
		if (REDIRECT_AFTER_ACTION) {
			...
			header('Location: meet_them.php?par=send');
			exit;
		} else {
			$_GET['par'] = 'send';
			SearchTable($res['err']);
...
function SearchTable($err="")
{
	...
	$profile_percent = new Percent($config, $dbconn, $user[ AUTH_ID_USER ]);
	
	$descr_perc = $profile_percent->GetSectionPercent(6);

	if ($descr_perc < 75) {
		$form["not_fill"] = str_replace("[percent]", intval($descr_perc),$lang["err"]["back_to_perfect_match"]);
		$smarty->assign("empty", "2");
		$form["perfect_link"] = "./myprofile.php?sel=3";
	} else {
		if ($use_session == 0) {
			unset($_SESSION["id_arr"]);	// unset session data
			$match_arr = GetWantToMeetThemList($user[ AUTH_ID_USER ]);
			$id_arr = count($match_arr)?$match_arr["id_arr"]:array();
			$_SESSION["id_arr"] = count($match_arr)?$match_arr["id_arr"]:array();
			$_SESSION["percent_arr"] = count($match_arr)?$match_arr["percent_arr"]:array();

			if(count($_SESSION["percent_arr"])>0){
		...
		if ($num_records > 0) {
			...
			if (strlen($user_str) > 0) {
				...
				foreach ($pages_arr as $v) {
					...
					//links
					$search[$i]["email_link"] = "mailbox.php?sel=fs&id=".$row["id"]."&search_type=mt";
					$search[$i]["profile_link"] = "viewprofile.php?id=".$row["id"]."&search_type=mt"."&page=".$page;
					...
					if ($search[$i]['hotlisted'] == 0 && $search[$i]['blacklisted'] == 0) {
						if ($settings['use_friend_types']) {
							...
							$search[$i]['add_hotlist_link'] = './meet_them.php?sel=addlist&amp;id='.$row['id'].'&amp;page='.$page;
						}
					}
					
					// blacklist link
					if ($search[$i]['hotlisted'] == 0 && $search[$i]['connected_status'] != CS_CONNECTED && $search[$i]['blacklisted'] == 0) {
						$search[$i]['add_blacklist_link'] = './meet_them.php?sel=blacklist&amp;id='.$row['id'].'&amp;page='.$page;
					}
					...
					if ($search[$i]['connected_status'] == CS_NOTHING && $search[$i]['blacklisted'] == 0) {
						if ($settings['use_friend_types']) {
							...
							$search[$i]['add_connection_link'] = './meet_them.php?sel=addconnection&amp;id='.$row['id'].'&amp;page='.$page;
						}
					}
	...
	$smarty->display(TrimSlash($config["index_theme_path"])."/meet_them_table.tpl");
	exit;
}
?>