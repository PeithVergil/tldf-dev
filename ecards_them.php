...
switch ($sel) {
	case 'addhotlist':
		...
		if (REDIRECT_AFTER_ACTION) {
			...
			header('Location: ecards_them.php?par=send');
			exit;
		} else {
			$_GET['par'] = 'send';
			SearchTable($res['err']);
	...
	if ($num_records > 0)
	{
		...
		$strSQL =
			'SELECT u.id, u.fname, u.phone, SUBSTRING(u.comment, 1, 165) AS comment, u.gender,
			   ...
			   FROM '.USERS_TABLE.' u
		 INNER JOIN '.ECARDS_ORDERS_TABLE.' l ON l.id_user_to = u.id AND l.id_user = ? AND l.status IN ("approved", "readed")
...
#			$search[$i]['completion']	= $percent->GetAllPercentForUser($row['id']);
	...
	$smarty->display(TrimSlash($config['index_theme_path']).'/ecards_them.tpl');
	exit;
}
?>