...
switch ($sel) {
	case 'addhotlist':
		...
		if (REDIRECT_AFTER_ACTION) {
			...
			header('Location: visit_my_page.php?par=send');
			exit;
		} else {
			$_GET['par'] = 'send';
			SearchTable($res['err']);
	...
	if ($num_records > 0)
	{
		...
		$strSQL =
			'SELECT u.id, u.fname, u.phone, SUBSTRING(u.comment, 1, 165) AS comment, u.gender,
			   ...
			   FROM '.USERS_TABLE.' u
		 INNER JOIN '.PROFILE_VISIT_TABLE.' l ON l.id_visiter = u.id AND l.id_user = ?
...
#			$search[$i]['completion']	= $percent->GetAllPercentForUser($row['id']);
	...
	$smarty->display(TrimSlash($config['index_theme_path']).'/visit_my_page_table.tpl');
	exit;
}
?>